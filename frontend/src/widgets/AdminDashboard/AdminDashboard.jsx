...
const S = {
  glass: { background: "rgba(255,255,255,0.055)", border: "1px solid rgba(255,255,255,0.10)", borderRadius: 16 },
  label: { fontSize: "0.7rem", letterSpacing: "2px", textTransform: "uppercase", color: "rgba(245,240,232,0.3)", marginBottom: 12, display: "block" },
  row: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.05)", fontSize: "0.875rem" },
};
...
export default function AdminDashboard() {
  ...
  if (loading) return <div style={{ padding: 32, color: "rgba(245,240,232,0.3)", fontFamily: "Vazirmatn, sans-serif" }}>در حال بارگذاری…</div>;

  return (
    <div style={{ padding: 28, fontFamily: "Vazirmatn, sans-serif", direction: "rtl", color: "#F5F0E8" }}>
      <span style={S.label}>داشبورد مدیریتی</span>
      <h2 style={{ fontSize: "1.2rem", fontWeight: 800, marginBottom: 24 }}>دکتر اکبرزاده</h2>
        ...
        {therapists.map(t => (
          <button key={t.id} onClick={() => setSelected(t)}
            style={{ padding: "12px 14px", borderRadius: 12, textAlign: "right", fontFamily: "inherit", cursor: "pointer", transition: "all 0.2s",
              background: selected?.id === t.id ? "rgba(45,212,172,0.1)" : "rgba(255,255,255,0.04)",
              border: `1px solid ${selected?.id === t.id ? "rgba(45,212,172,0.35)" : "rgba(255,255,255,0.10)"}`,
              color: "#F5F0E8" }}>
            <div style={{ fontWeight: 600, fontSize: "0.875rem", marginBottom: 3 }}>{t.name}</div>
            <div style={{ fontSize: "0.72rem", color: "rgba(245,240,232,0.35)" }}>{t.years_active} سال سابقه</div>
          </button>
        ))}
      </div>
      ...
      {selected && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ ...S.glass, padding: 22 }}>
            ...
            {summaries.length === 0
              ? <p style={{ color: "rgba(245,240,232,0.3)", fontSize: "0.83rem" }}>داده‌ای یافت نشد.</p>
              : summaries.map(s => (
              ...
              {cancels.filter(c => !c.decided_by_head).map(c => (
                <div key={c.id} style={S.row}>
                  ...
                  <span style={{ fontSize: "0.72rem", padding: "3px 10px", borderRadius: 100,
                    background: c.salary_safe ? "rgba(45,212,172,0.1)" : "rgba(239,68,68,0.1)",
                    border: `1px solid ${c.salary_safe ? "rgba(45,212,172,0.25)" : "rgba(239,68,68,0.2)"}`,
                    color: c.salary_safe ? "#2DD4AC" : "#fca5a5" }}>
                    {c.salary_safe ? "ایمن" : "نیاز به کسر"}
                  </span>
                  <span style={{ fontSize: "0.72rem", color: "rgba(245,240,232,0.3)" }}>از Admin Panel تصمیم بگیرید</span>
                </div>
              ))}
            </div>
          )}
