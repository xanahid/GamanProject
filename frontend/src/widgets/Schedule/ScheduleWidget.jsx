...
// Slot appearance — dark-theme aware
const slotConfig = (status, mode) => {
  if (mode === "client") {
    if (status === "booked") return {
      bg: "rgba(239,68,68,0.08)", border: "rgba(239,68,68,0.25)", color: "#fca5a5",
      label: "رزرو شده", disabled: true,
    };
    if (status === "pending_payment") return {
      bg: "rgba(232,184,75,0.08)", border: "rgba(232,184,75,0.3)", color: "#E8B84B",
      label: "معمولاً رزرو می‌شود — امکان رزرو", disabled: false,
    };
    return {
      bg: "rgba(255,255,255,0.04)", border: "rgba(255,255,255,0.10)", color: "#F5F0E8",
      label: "آزاد", disabled: false,
    };
  }
  // therapist view
  if (status === "booked") return {
    bg: "rgba(45,212,172,0.08)", border: "rgba(45,212,172,0.25)", color: "#2DD4AC",
    label: "رزرو شده", disabled: true,
  };
  if (status === "pending_payment") return {
    bg: "rgba(232,184,75,0.08)", border: "rgba(232,184,75,0.3)", color: "#E8B84B",
  ...
  return {
    bg: "rgba(255,255,255,0.04)", border: "rgba(255,255,255,0.10)", color: "rgba(245,240,232,0.6)",
    label: "آزاد", disabled: true,
  };
};

const LEGENDS_CLIENT = [
  { bg: "rgba(255,255,255,0.06)", border: "rgba(255,255,255,0.12)", label: "آزاد" },
  { bg: "rgba(232,184,75,0.1)", border: "rgba(232,184,75,0.3)", label: "معمولاً رزرو می‌شود" },
  { bg: "rgba(239,68,68,0.08)", border: "rgba(239,68,68,0.2)", label: "رزرو شده" },
];
const LEGENDS_THERAPIST = [
  { bg: "rgba(255,255,255,0.06)", border: "rgba(255,255,255,0.12)", label: "آزاد" },
  { bg: "rgba(232,184,75,0.1)", border: "rgba(232,184,75,0.3)", label: "در انتظار پرداخت" },
  { bg: "rgba(45,212,172,0.08)", border: "rgba(45,212,172,0.25)", label: "رزرو شده" },
];

// ── shared style tokens ───────────────────────────────────────────────────────
const S = {
  glass:       { background: "rgba(255,255,255,0.055)", border: "1px solid rgba(255,255,255,0.10)", borderRadius: 14 },
  glassStrong: { background: "rgba(255,255,255,0.09)",  border: "1px solid rgba(255,255,255,0.14)", borderRadius: 20, backdropFilter: "blur(24px)" },
  btn: {
    background: "#2DD4AC", color: "#0D0D0F", border: "none", borderRadius: 10,
    padding: "11px 24px", fontWeight: 700, fontSize: "0.875rem", cursor: "pointer",
  ...
  label: { fontSize: "0.72rem", letterSpacing: "2px", textTransform: "uppercase", color: "rgba(245,240,232,0.3)", marginBottom: 12, display: "block" },
  error: { fontSize: "0.82rem", color: "#fca5a5", background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 10, padding: "10px 14px", marginBottom: 14 },
  row: { display: "grid", gridTemplateColumns: "84px 1fr", gap: 12, alignItems: "center", marginBottom: 8 },
  timeCell: { fontSize: "0.75rem", color: "rgba(245,240,232,0.3)", textAlign: "center", fontFamily: "monospace", direction: "ltr" },
};
...
export default function ScheduleWidget({ therapistId, therapistName, mode, lang, sessionPrice }) {
  ...
  if (step === "confirm" && selected) return (
    <div style={{ padding: 32, fontFamily: "Vazirmatn, sans-serif", direction: "rtl" }}>
      ...
      <h2 style={{ fontSize: "1.2rem", fontWeight: 800, color: "#F5F0E8", marginBottom: 20 }}>جزئیات جلسه</h2>

      <div style={{ ...S.glassStrong, padding: 20, marginBottom: 20 }}>
        {[
          ...
          <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.06)", fontSize: "0.875rem" }}>
      ...
      {/* Weekly toggle */}
      <div style={{ ...S.glass, padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <div style={{ fontSize: "0.875rem", color: "#F5F0E8", fontWeight: 600 }}>رزرو ثابت هفتگی</div>
          <div style={{ fontSize: "0.75rem", color: "rgba(245,240,232,0.35)", marginTop: 3 }}>این زمان همیشه برای شما حفظ می‌شود</div>
        </div>
        <button onClick={() => setMakeWeekly(!makeWeekly)}
          style={{ width: 44, height: 24, borderRadius: 12, background: makeWeekly ? "#2DD4AC" : "rgba(255,255,255,0.12)", border: "none", cursor: "pointer", position: "relative", transition: "background 0.25s" }}>
          <span style={{ position: "absolute", top: 2, width: 20, height: 20, background: "#fff", borderRadius: "50%", transition: "all 0.25s",
  ...
  if (step === "cards") return (
    <div style={{ padding: 32, fontFamily: "Vazirmatn, sans-serif", direction: "rtl" }}>
      ...
      <h2 style={{ fontSize: "1.2rem", fontWeight: 800, color: "#F5F0E8", marginBottom: 6 }}>انتخاب کارت</h2>
      <p style={{ fontSize: "0.83rem", color: "rgba(245,240,232,0.4)", marginBottom: 24 }}>کارت پرداخت را انتخاب کنید</p>
        ...
        {cards.map(card => (
          <button key={card.id} onClick={() => setChosenCard(card.id)}
            style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center",
              ...
              background: chosenCard === card.id ? "rgba(45,212,172,0.1)" : "rgba(255,255,255,0.04)",
              border: `1px solid ${chosenCard === card.id ? "rgba(45,212,172,0.4)" : "rgba(255,255,255,0.10)"}`,
              transition: "all 0.2s",
            }}>
            ...
            {card.is_default && <span style={{ fontSize: "0.68rem", padding: "2px 10px", borderRadius: 100, background: "rgba(45,212,172,0.12)", color: "#2DD4AC", border: "1px solid rgba(45,212,172,0.25)" }}>پیش‌فرض</span>}
          </button>
        ))}
        {cards.length === 0 && (
          <p style={{ color: "rgba(245,240,232,0.35)", fontSize: "0.83rem", textAlign: "center", padding: "16px 0" }}>
      ...
      <p style={{ textAlign: "center", fontSize: "0.75rem", color: "rgba(245,240,232,0.25)", marginTop: 10 }}>پس از کلیک به درگاه امن شاپرک منتقل می‌شوید</p>
    </div>
  );
    ...
    <div style={{ padding: 32, fontFamily: "Vazirmatn, sans-serif", direction: "rtl", color: "#F5F0E8" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, paddingBottom: 24, borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
        <button onClick={() => changeDay(isRtl ? 1 : -1)}
          style={{ width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 10, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.10)", color: "#F5F0E8", cursor: "pointer", fontSize: "1.1rem", transition: "all 0.2s" }}>
          {isRtl ? "›" : "‹"}
        </button>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontWeight: 700, fontSize: "1rem" }}>{dayLabel()}</div>
          <div style={{ fontSize: "0.8rem", color: "rgba(245,240,232,0.4)", marginTop: 3, direction: "ltr" }}>{dateStr}</div>
        </div>
        <button onClick={() => changeDay(isRtl ? -1 : 1)}
          style={{ width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 10, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.10)", color: "#F5F0E8", cursor: "pointer", fontSize: "1.1rem", transition: "all 0.2s" }}>
          {isRtl ? "‹" : "›"}
      ...
      {loading ? (
        <div>{[...Array(7)].map((_,i) => (
          <div key={i} style={{ ...S.row }}>
            <div style={{ height: 14, borderRadius: 4, background: "rgba(255,255,255,0.05)" }}></div>
            <div style={{ height: 52, borderRadius: 12, background: "rgba(255,255,255,0.05)", opacity: 1 - i * 0.1 }}></div>
          </div>
        ))}</div>
      ) : slots.length === 0 ? (
        <div style={{ textAlign: "center", color: "rgba(245,240,232,0.3)", padding: "48px 0", fontSize: "0.875rem" }}>این روز وقت آزاد ندارد.</div>
      ) : (
        <div>
          {slots.map(slot => {
                ...
                <div style={{ position: "relative" }}>
                  <button
                    ...
                    style={{
                      ...
                      background: isSelected ? "rgba(45,212,172,0.12)" : cfg.bg,
                      border: `1px solid ${isSelected ? "rgba(45,212,172,0.4)" : cfg.border}`,
                  ...
                  {isTherapist && hovered === slot.id && slot.patient_name && (
                    <div style={{ position: "absolute", bottom: "calc(100% + 6px)", right: 0, background: "rgba(13,13,15,0.96)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, padding: "8px 14px", fontSize: "0.78rem", whiteSpace: "nowrap", zIndex: 20, backdropFilter: "blur(16px)" }}>
      ...
      {/* Legend */}
      <div style={{ display: "flex", gap: 20, marginTop: 24, flexWrap: "wrap", paddingTop: 20, borderTop: "1px solid rgba(255,255,255,0.07)" }}>
        {(isTherapist ? LEGENDS_THERAPIST : LEGENDS_CLIENT).map(item => (
          <div key={item.label} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: "0.75rem", color: "rgba(245,240,232,0.35)" }}>
      ...
      {notifySlot && (
        <div onClick={() => setNotifySlot(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 24, backdropFilter: "blur(4px)" }}>
          <div onClick={e => e.stopPropagation()} style={{ ...S.glassStrong, padding: 32, maxWidth: 380, width: "100%", fontFamily: "Vazirmatn, sans-serif", direction: "rtl" }}>
            <div style={{ fontSize: "1rem", fontWeight: 700, color: "#F5F0E8", marginBottom: 10 }}>🔔 اطلاع‌رسانی</div>
            <p style={{ fontSize: "0.85rem", color: "rgba(245,240,232,0.5)", lineHeight: 1.8, marginBottom: 24 }}>
